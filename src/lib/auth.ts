...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
      ...
      CredentialsProvider({
        ...
        async authorize(credentials) {
          ...
          try {
            ...
            if (error) {
              console.log('Dev-login: Supabase error:', error.message)
              return null
            }
    ...
    // EmailProvider temporarily disabled due to Gmail credential issues
    ...
    signIn: async ({ user, account, profile }) => {
      try {
        ...
        if (error && error.code !== 'PGRST116') {
          logger.error('Error fetching user in signIn callback', {
            email: user.email,
            error: error.message
          })
        ...
        if (existingUser && existingUser.createdAt) {
          const timeSinceCreation = Date.now() - new Date(existingUser.createdAt).getTime()
