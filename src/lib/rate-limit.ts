...
class PostgreSQLRateLimiter {
  ...
  private async checkWithPostgreSQL(key: string, limit: number, windowMs: number, now: number): Promise<RateLimitResult> {
    ...
    if (error) {
      throw new Error(`Failed to fetch rate limit data: ${error.message}`)
    }
    ...
    if (upsertError) {
      throw new Error(`Failed to upsert rate limit data: ${upsertError.message}`)
    }
...
export const RATE_LIMITS = {
  ...
  GENERAL: { limit: 100, windowMs: 15 * 60 * 1000 }, // 100 requests per 15 minutes (alias for API)
...
// API rate limit middleware (alias for compatibility)
export const withAPIRateLimit = (config: { limit: number; windowMs: number }, keyGenerator?: (req: any) => string) => {
  return (handler: any) => {
    return async (req: any, res: any) => {
      try {
        const key = keyGenerator ? keyGenerator(req) : getRateLimitKey(req, 'API')
        const result = await rateLimiter.check(key, config.limit, config.windowMs)
